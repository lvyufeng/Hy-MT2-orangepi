...
// Per-row RMSNorm specialized for hidden=2048 fp16. One block per row; gamma
// loaded once per block. Mirrors rms_norm128_custom but with the 2048-hidden
// dim used by Hy-MT2 input_norm / post_attention_norm / final_norm.

class KernelRmsNorm2048Custom {
public:
    __aicore__ inline KernelRmsNorm2048Custom() {}
...
private:
    static constexpr uint32_t DIM = 2048;
    static constexpr float INV_DIM = 1.0f / 2048.0f;
...
extern "C" __global__ __aicore__ void rms_norm2048_custom(GM_ADDR x, GM_ADDR gamma, GM_ADDR out,
                                                           GM_ADDR workspace, GM_ADDR tiling) {
    GET_TILING_DATA(tiling_data, tiling);
    KernelRmsNorm2048Custom op;
