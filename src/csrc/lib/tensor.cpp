...
void Tensor::copy_from_host(const void* src, size_t bytes) {
    ...
    check_acl(aclrtMemcpy(buffer_.data(), size_bytes(), src, bytes, ACL_MEMCPY_HOST_TO_DEVICE),
              "aclrtMemcpy H2D");
}

void Tensor::copy_to_host(void* dst, size_t bytes) const {
    check_acl(aclrtMemcpy(dst, bytes, buffer_.data(), bytes, ACL_MEMCPY_DEVICE_TO_HOST),
              "aclrtMemcpy D2H");
}
