...
> 状态（Phase 0）：当前是脚手架阶段。`AclContext` + `Tensor` 已经能在 NPU 上完成
> fp16 H2D/D2H round-trip。自定义算子、decoder layer、完整 LM 会在后续阶段落地。
...
| 1 | safetensors loader（BF16→FP16）+ tokenizers-cpp | |
