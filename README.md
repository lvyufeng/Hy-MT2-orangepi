...
The custom cube path improves common decode M=1 projection/MLP matmuls by roughly 40-50x versus the original public-aclnn transposed-view path, and the custom decode-attention path cuts the current 30-token decode bench to about 1.46 tok/s. The remaining bottlenecks are RoPE/RMSNorm launch overheads, temporary tensor copies, and the still-FP16 weight bandwidth.
